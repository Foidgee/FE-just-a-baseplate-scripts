...
local function sendChatMessage(msg)
    ...
    -- Nuevo sistema de chat (TextChatService)
    if TextChatService.ChatVersion == Enum.ChatVersion.TextChatService then
        ...
        if textChannels then
            ...
            if generalChannel then
                ...
                warn("Error al enviar por TextChatService:", err)
            end
        end
    end

    -- Sistema clásico de chat (DefaultChatSystemChatEvents)
    ...
    if chatEvents then
        ...
        if sayMessageRequest then
            ...
            if success then return end
            warn("Error al enviar por DefaultChatSystemChatEvents:", err)
        end
    end

    -- Si no pudo enviar, advertir
    warn("El sistema de chat no es compatible con este script.")
end

-- Arreglado: agregadas las comas faltantes
local lines = {
    "-net",
    "made by foidgee a.k.a coolfescriptmaker",
    "enjoy 🤑"
}

-- Enviar cada línea con una pausa
task.wait(1)
for _, line in ipairs(lines) do
    sendChatMessage(line)
    task.wait(2.0) -- pequeña pausa para evitar flood tag
end
...
con = renderstepped:Connect(function(deltaTime)
    ...
    if vel.Magnitude > 2 then
        ...
        elseif mode == "salute" then

RootJoint.C0 = RootJoint.C0:Lerp(cf(0, 0, 0) * euler(-1.5708, 0, -math.pi), deltaTime)
Neck.C0 = Neck.C0:Lerp(cf(0, 1, 0) * euler(-1.5708, 0, -math.pi), deltaTime)

LeftShoulder.C0 = LeftShoulder.C0:Lerp(cf(-1, 0.5, 0) * euler(1.5708, -1.5708, 1.5708), deltaTime)


RightShoulder.C0 = RightShoulder.C0:Lerp(
    cf(1, 0.8, 0) * euler(-1.35, 0.25, math.pi),
    deltaTime
)

RightHip.C0 = RightHip.C0:Lerp(cf(1, -1, 0) * euler(1.5708, 1.5708, -1.5708), deltaTime)
LeftHip.C0 = LeftHip.C0:Lerp(cf(-1, -1, 0) * euler(1.5708, -1.5708, 1.5708), deltaTime)
